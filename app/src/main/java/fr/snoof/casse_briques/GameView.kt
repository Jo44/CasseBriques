...
// Class GameView
class GameView(context: Context, private var screenWidth: Int, private var screenHeight: Int) : SurfaceView(context), Runnable {

    // Class Brick
    private class Brick(var x: Float, var y: Float, var width: Float, var height: Float, var color: Int) {
    ...
    constructor(context: Context, attrs: AttributeSet?) : this(context, 0, 0) {
        screenWidth = 1000
        screenHeight = 2000
    }
    constructor(context: Context, attrs: AttributeSet?, defStyleAttr: Int) : this(context, 0, 0) {
        screenWidth = 1000
        screenHeight = 2000
    }
        ...
        if (allBricksBroken()) {
            ...
            for (brick in bricks) {
                if (!brick!!.isBroken) {
                    ...
                    val distance = sqrt((ballX - closestX).toDouble().pow(2.0) + (ballY - closestY).toDouble().pow(2.0))
    ...
    private fun displayGameOverMessage(message: String) {
        ...
        canvas.drawText(message, canvas.width / 2 - textWidth / 2,
    ...
    private fun draw() {
        if (holder!!.surface.isValid) {
            ...
            canvas.drawRect(0F, 0F, screenWidth.toFloat(), (screenHeight - slide + 30).toFloat(), paint!!)

            // Dessine la barre de fin de jeu
            paint!!.color = Color.WHITE
            canvas.drawRect(0F, (screenHeight - slide + 30).toFloat(), screenWidth.toFloat(), (screenHeight - slide + 30).toFloat() + 2, paint!!)
